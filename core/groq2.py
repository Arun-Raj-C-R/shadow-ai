...
class CoderAgent:
    ...
    def generate_and_execute_code(self, manager_plan, user_request, use_visualization=True):
        ...
        print("🔄 Coder Agent starting execution...")
        
        for attempt in range(self.max_attempts):
            self.execution_attempts = attempt + 1
            print(f"\n🔄 Attempt {self.execution_attempts}/{self.max_attempts}")
            ...
            if execution_result["success"]:
                ...
                print("🔄 Learning from error and retrying...")
...
class ReportGenerator:
    ...
    def generate_markdown_report(self, filename="physics_report.md"):
        """Generate markdown report of all solved problems"""
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("# Physics Problem Solving Report\n\n")
            f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            f.write(f"Total Problems Solved: {len(self.results_history)}\n\n")
            f.write("---\n\n")
            
            for idx, entry in enumerate(self.results_history, 1):
                f.write(f"## Problem {idx}\n\n")
                f.write(f"**Question:** {entry['problem']}\n\n")
                f.write(f"**Status:** {entry['result']['status'].upper()}\n\n")
                
                if entry['result']['status'] == 'success':
                    f.write(f"**Attempts:** {entry['result']['attempts']}\n\n")
                    f.write("**Output:**\n```\n")
                    f.write(entry['result']['output'])
                    f.write("\n```\n\n")
                    
                    f.write("**Generated Code:**\n```python\n")
                    f.write(entry['result']['generated_code'])
                    f.write("\n```\n\n")
                else:
                    f.write(f"**Error:** {entry['result']['error']}\n\n")
...
class AICodeOrchestrator:
    ...
    def process_request(self, user_request, use_visualization=True):
        ...
        print("🚀 Starting AI Manager-Agent System")
        print(f"📝 User Request: {user_request}")
        print("\n" + "="*50)
        
        # Step 1: Manager creates execution plan
        print("👔 Manager Agent is creating execution plan...")
        manager_plan = self.manager.create_execution_plan(user_request)
        print("✅ Manager Plan Created")
        
        print("\n" + "="*50)
        
        # Step 2: Coder executes with retry logic
        print("👨‍💻 Coder Agent is generating and executing code...")
        ...
        if result["status"] == "success":
            ...
            # Get manager's analysis
            analysis = self.manager.analyze_result(result)
            if analysis:
                print("\n" + "="*50)
                print("🧠 MANAGER'S ANALYSIS:")
                print(analysis)
        else:
        ...
        self.report_gen.add_result(user_request, result)
        
        return result
    
    def save_report(self, filename="physics_report.md"):
        """Save report of all problems"""
        self.report_gen.generate_markdown_report(filename)
...
def test_physics_problems():
    orchestrator = AICodeOrchestrator()
    
    test_problems = [
        "Calculate the gravitational force between Earth (5.97e24 kg) and Moon (7.35e22 kg) separated by 384,400 km",
        "Simulate projectile motion for an object launched at 45 degrees with initial velocity 20 m/s and plot the trajectory",
        "Calculate energy levels of hydrogen atom for n=1,2,3,4,5 and visualize them",
        "Plot electric field lines for a dipole configuration",
        "Calculate and plot the wave function for quantum harmonic oscillator (n=0,1,2,3)"
    ]
    ...
    for i, problem in enumerate(test_problems, 1):
        ...
        result = orchestrator.process_request(problem, use_visualization=True)
    ...
    # Save report
    orchestrator.save_report("test_physics_report.md")
...
def interactive_chat():
    ...
    print("="*60)
    print("🤖 AI PHYSICS ASSISTANT - INTERACTIVE MODE")
    print("="*60)
    print("\nCommands:")
    print("  • Type your physics problem to solve it")
    print("  • 'test' - Run test problems")
    print("  • 'report' - Save report of solved problems")
    ...
    while True:
        ...
        if user_input.lower() == 'exit':
            print("\n👋 Goodbye! Thanks for using AI Physics Assistant!")
            break
        ...
        print("\n" + "="*60)
        result = orchestrator.process_request(user_input, use_visualization=True)
        
        if result["status"] == "success":
            print(f"\n🎉 Problem solved successfully!")
            
            show_code = input("\n📝 Show generated code? (y/n): ").lower()
            if show_code == 'y':
                print("\n" + "="*60)
            ...
            print("Try rephrasing or breaking it into simpler parts.")
...
def main():
    print("\n" + "="*60)
    print("🔬 AI PHYSICS PROBLEM SOLVER WITH VISUALIZATION")
    print("="*60)
    print("\nSelect Mode:")
    print("  1. Interactive Chat Mode")
    print("  2. Run Test Problems")
    print("  3. Exit")
    print("="*60)
